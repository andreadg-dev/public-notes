...
function calculate(
  a: number,
  b: number,
  calcFunc: (num1: number, num2: number) => number,
) {
  calcFunc(a, b);
...
let mainAppAdmin2: AppAdminType = {
  username: "jane123",
  permissions: ["create", "delete"],
};
```
...
function loginApp(u: NormalUser | AdminUser) {
  if (typeof u === NormalUser) {
    //❌ Error: typeof does not work with custom type aliases/interfaces
    // do something
  }
}
```
